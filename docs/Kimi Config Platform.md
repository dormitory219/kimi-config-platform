...
- **后端**: Go 1.23 + Gin + starlark-go + go-git
...
1. 开发者在 React 前端编辑 `.star` 脚本 → 保存到工作区
2. 点击"发布" → Go 后端 `git commit && git push`
3. App 启动时请求 `GetConfig(version, lang, region)` → Go 执行对应 Starlark 脚本 → 返回 protobuf JSON
